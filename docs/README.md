...
1. A ruling you'd defend if someone reopened it? → [`decisions/`](decisions/) — dated,
   never edited.
2. Third-party facts you keep to look up (a framework, an external API/CLI)? → `vendor/` —
   link-first, mark provenance. *(No vendor material here yet; add the folder when there
   is.)*
